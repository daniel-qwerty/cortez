...
jQuery(document).ready(function ($) {
    ...
    $.datepicker.setDefaults({
        onSelect: function (val, target) {
            ...
            var hasta = $("#checkout").val();
            var n_date = new Date(val);
            var new_date = getDate(n_date.getUTCFullYear(), n_date.getUTCMonth() + 1, n_date.getUTCDate(), 0);
            ...
            if (new_date < today_date) {
                ...
                if (target.id == "checkin") {
                    $("#checkout").val(sumaFecha(1, val));
                    desde = $("#checkin").val();
                    hasta = $("#checkout").val();
                } else if (target.id == "checkout" && $('#checkin').val() == '') {
                    $("#checkin").val(sumaFecha(-1, val))
                    desde = $("#checkin").val();
                    hasta = $("#checkout").val();
                } else if (target.id == "checkout" && $('#checkin').val() > $('#checkout').val()) {
                    $("#checkin").val(sumaFecha(-1, val))
                    desde = $("#checkin").val();
                    hasta = $("#checkout").val();
            ...
            var desde2 = $("#checkin2").val();
            var hasta2 = $("#checkout2").val();
            var n_date2 = new Date(val);
            var new_date2 = getDate(n_date2.getUTCFullYear(), n_date2.getUTCMonth() + 1, n_date2.getUTCDate(), 0);

            var t_date2 = new Date();
            ...
            if (new_date2 < today_date2) {
                ...
                if (target.id == "checkin2") {
                    $("#checkout2").val(sumaFecha(1, val));
                    desde2 = $("#checkin2").val();
                    hasta2 = $("#checkout2").val();
                } else if (target.id == "checkout2" && $('#checkin2').val() == '') {
                    $("#checkin2").val(sumaFecha(-1, val))
                    desde2 = $("#checkin2").val();
                    hasta2 = $("#checkout2").val();
                } else if (target.id == "checkout2" && $('#checkin2').val() > $('#checkout2').val()) {
                    $("#checkin").val(sumaFecha(-1, val))
                    desde2 = $("#checkin2").val();
                    hasta2 = $("#checkout2").val();
    ...
    function sumaFecha(dias, fecha) {
        ...
        var mYear = fecha.getUTCFullYear();
        var mMonth = (fecha.getUTCMonth() + 1);
        var mDay = fecha.getUTCDate();
        return mYear + '-' + ((mMonth < 10) ? "0" + mMonth : mMonth) + '-' + ((mDay < 10) ? "0" + mDay : mDay);
    }

    function getDate(_year, _month, _day, offset) {
        var x = new Date(_year + "-" + _month + "-" + _day);
        var y = new Date(x.valueOf() + offset + x.getTimezoneOffset() * 60000);
        return y;
    }
...
function sendContact(mensajeError,mensajeOk) {
    ...
    if (name === "" || email === "" || message === "") {
        ...
        $.ajax({
            type: "POST",
            url: "http://digitalmindsbolivia.com/cortez/Service/Contact/Save",
            data: {
                Name: name,
                Email: email,
                Message: message
...
function sendRequest(mensajeError,mensajeOk) {
    ...
    if (name === "" || email === "" || message === "") {
        ...
        $.ajax({
            type: "POST",
            url: "http://digitalmindsbolivia.com/cortez/Service/Request/Save",
            data: {
                Name: name,
                Email: email,
                Phone: phone,
            ...
            $('#formContact #type').val("");
            $('#formContact #item').val("");
        }).error(function () {
...
function sendReserva(mensaje,title,text,btnOk,btnOpciones) {
    ...
    if (tipo === "" || checkin === "" || checkout === "") {
        ...
        $.ajax({
            type: "POST",
            url: "http://digitalmindsbolivia.com/motorreservas/reserva/default/service",
            data: {
                llegada: checkin,
                salida: checkout,
                tipoHabitacion: tipo
